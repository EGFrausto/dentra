...
export default function Appointments() {
  ...
  const toggleReminder = id => {
    ...
    aStore.update(id, { reminder_sent: !a.reminder_sent }); reload();
  };
            ...
            {cells.map((day, i) => {
              ...
              return (
                <button key={ds} onClick={() => setSel(ds)}
                  ...
                  {day}
                  {count > 0 && <span className={`absolute bottom-1 w-1.5 h-1.5 rounded-full ${isSel ? 'bg-white' : 'bg-violet-400'}`} />}
                </button>
              );
            })}
          </div>
      ...
      <div className="w-80 border-l border-slate-100 bg-white overflow-y-auto p-5">
        ...
        {selApts.length === 0
          ...
          : <div className="space-y-3">
              {selApts.map(a => (
                <div key={a.id} className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => toggleReminder(a.id)}
                      className={`flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-lg font-medium transition-colors ${a.reminder_sent ? 'bg-teal-100 text-teal-700' : 'bg-slate-100 text-slate-500'}`}>
        ...
        <Modal title={editing ? 'Editar Cita' : 'Nueva Cita'} onClose={() => setModal(false)}>
          <div className="p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><label className="label">Hora *</label><input type="time" value={form.time} onChange={f('time')} className="input-field" /></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="label">Duración</label>
                <select value={form.duration} onChange={f('duration')} className="input-field">
            ...
            <div><label className="label">Tratamiento</label>
              <select value={form.treatment} onChange={f('treatment')} className="input-field">
                <option value="">Seleccionar tratamiento...</option>
                {TREATMENTS.map(t => <option key={t}>{t}</option>)}
              </select>
            </div>
            <div><label className="label">Notas</label><textarea value={form.notes} onChange={f('notes')} placeholder="Notas adicionales..." rows={3} className="input-field resize-none" /></div>
          </div>
