...
const STATUS_COLORS = {
  'Confirmada': { badge:'badge badge-teal',   card:'border-l-4 border-teal-400' },
  'Programada': { badge:'badge badge-indigo', card:'border-l-4 border-indigo-400' },
  'Completada': { badge:'badge badge-slate',  card:'border-l-4 border-slate-300' },
  'Cancelada':  { badge:'badge badge-red',    card:'border-l-4 border-red-400' },
};
const CAL_COLORS = { 'Confirmada':'bg-teal-400', 'Programada':'bg-indigo-400', 'Completada':'bg-slate-300', 'Cancelada':'bg-red-400' };
...
export default function Appointments() {
  ...
  const confirmDelete = () => { aStore.remove(confirmDel); reload(); setConfirmDel(null); };
      ...
      <div className="flex-1 p-8 overflow-auto">
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-5">
            <button onClick={()=>setCur(new Date(yr,mo-1,1))} className="p-1.5 hover:bg-slate-100 rounded-lg"><ChevronLeft size={18}/></button>
            <h2 className="font-bold text-slate-700">{MONTHS[mo]} {yr}</h2>
            <button onClick={()=>setCur(new Date(yr,mo+1,1))} className="p-1.5 hover:bg-slate-100 rounded-lg"><ChevronRight size={18}/></button>
          </div>
          ...
          <div className="grid grid-cols-7 gap-1">
            {cells.map((day,i)=>{
              ...
              const dayApts=byDate(ds), isToday=ds===today, isSel=ds===sel;
              return (
                <button key={ds} onClick={()=>setSel(ds)}
                  className={`aspect-square flex flex-col items-center justify-center rounded-xl text-sm transition-all relative font-medium ${isSel?'bg-violet-600 text-white shadow-md shadow-violet-200':isToday?'bg-violet-50 text-violet-700 ring-1 ring-violet-200':'hover:bg-slate-50 text-slate-600'}`}>
                  {day}
                  {dayApts.length>0&&<div className="absolute bottom-1 flex gap-0.5">{dayApts.slice(0,3).map((a,idx)=><span key={idx} className={`w-1.5 h-1.5 rounded-full ${isSel?'bg-white':(CAL_COLORS[a.status]||'bg-slate-400')}`}/>)}</div>}
                </button>
              );
            })}
          </div>
              ...
              return (
                <div key={a.id} className={`bg-slate-50 rounded-2xl p-4 border border-slate-100 ${sc.card}`}>
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={()=>toggleReminder(a.id)} className={`flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-lg font-medium transition-colors ${a.reminder_sent?'bg-indigo-100 text-indigo-700':'bg-slate-100 text-slate-500'}`}>
                      {a.reminder_sent?<Bell size={11}/>:<BellOff size={11}/>}{a.reminder_sent?'Recordatorio enviado':'Sin recordatorio'}
                    </button>
                    <div className="ml-auto flex">
                      <button onClick={()=>openEdit(a)} className="icon-btn"><Edit2 size={13}/></button>
                      <button onClick={()=>setConfirmDel(a.id)} className="icon-btn-danger"><Trash2 size={13}/></button>
                    </div>
                  </div>
                </div>
              );
        ...
        <Modal title={editing?'Editar Cita':'Nueva Cita'} onClose={()=>setModal(false)}>
          <div className="p-6 space-y-4">
            <div><label className="label">Paciente *</label>
              <Select value={form.patient_id} onChange={v=>sf('patient_id',v)} placeholder="Seleccionar paciente..." options={pts.map(p=>({value:p.id,label:p.name}))}/>
            </div>
