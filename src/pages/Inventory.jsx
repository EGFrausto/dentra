...
export default function Inventory() {
  ...
  const save = ()=>{
    ...
    editing ? store.update(editing,{...form,quantity:Number(form.quantity),min_stock:Number(form.min_stock)})
            : store.add({...form,quantity:Number(form.quantity),min_stock:Number(form.min_stock)});
    reload(); setModal(false);
  };

  const del = id=>{if(!confirm('¿Eliminar este artículo?'))return;store.remove(id);reload();};
    ...
    <div className="p-8">
      ...
      <div className="flex gap-3 mb-5">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar artículos..." className="input-field pl-10"/>
        </div>
        <div className="w-56">
          <Select value={filterCat} onChange={v=>setFilterCat(v)}
                ...
                <div key={item.id} className="grid grid-cols-5 items-center px-5 py-4 border-b border-slate-50 last:border-0 hover:bg-slate-50/80 transition-colors">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-0.5">
                      ...
                      <button onClick={()=>del(item.id)} className="icon-btn-danger"><Trash2 size={14}/></button>
                    </div>
                  </div>
                </div>
              );
        ...
        <Modal title={editing?'Editar Artículo':'Nuevo Artículo'} onClose={()=>setModal(false)}>
          <div className="p-6 space-y-4">
            ...
            <div><label className="label">Categoría</label>
              <Select value={form.category} onChange={v=>sf('category',v)} placeholder="Seleccionar..." options={CATEGORIES} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><label className="label">Unidad</label>
                <Select value={form.unit} onChange={v=>sf('unit',v)} options={UNITS} />
              </div>
            </div>
